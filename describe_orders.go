...
type DescribeOrdersResponse struct {
	...
	Request    *DescribeOrdersRequest `json:"request"`
}

// DescribeOrders returns orders based on query params. Start time is requred.
...
func (c Client) DescribeOrders(
	ctx context.Context,
	request *DescribeOrdersRequest,
) (*DescribeOrdersResponse, error) {
	...
	response := &DescribeOrdersResponse{Request: request}
	if err := get(ctx, c, path, queryParams, request, response); err != nil {
		return response, fmt.Errorf("unable to DescribeOrders: %w", err)
	}
