...
func TestDescribeWalletDepositInstructions(t *testing.T) {
	...
	testDescribeDepositInstructions(t, client, walletsResponse.Wallets[0].Id)
}

func testDescribeDepositInstructions(t *testing.T, client *prime.Client, walletId string) {

	ctx, cancel := context.WithTimeout(context.Background(), 10*time.Second)
	defer cancel()

	response, err := client.DescribeWalletDepositInstructions(ctx, &prime.DescribeWalletDepositInstructionsRequest{
		PortfolioId: client.Credentials.PortfolioId,
		Id:          walletId,
