...
type GetOpenOrdersResponse struct {
	...
	Request *GetOpenOrdersRequest `json:"request"`
}

// GetOpenOrders enables searching for open orders by product id.
...
func (c Client) GetOpenOrders(
	...
	response := &GetOpenOrdersResponse{Request: request}

	if err := get(ctx, c, path, queryParams, request, response); err != nil {
		return response, fmt.Errorf("unable to GetOpenOrders: %w", err)
	}
