...
type GetCommissionResponse struct {
	...
	Request    *GetCommissionRequest `json:"request"`
}

func (c Client) GetCommission(
	ctx context.Context,
	request *GetCommissionRequest,
) (*GetCommissionResponse, error) {

	path := fmt.Sprintf("/portfolios/%s/commission", request.PortfolioId)

	response := &GetCommissionResponse{Request: request}
