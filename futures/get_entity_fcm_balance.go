...
type GetEntityFcmBalanceRequest struct {
...
) (*GetEntityFcmBalanceResponse, error) {

	path := fmt.Sprintf("/entities/%s/futures/balance_summary", request.EntityId)

	response := &GetEntityFcmBalanceResponse{Request: request}
