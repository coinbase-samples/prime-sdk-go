...
func main() {
	...
	if len(os.Args) < 2 {
		log.Fatalf("transaction id is required as a command-line argument")
	}
	...
	response, err := transactionsSvc.GetTransactionTravelRuleData(context.Background(), request)
	if err != nil {
		log.Fatalf("unable to get transaction travel rule data: %v", err)
	}
