...
func main() {
	...
	request := &advancedtransfers.CreateAdvancedTransferRequest{
		...
		AdvancedTransfer: &model.AdvancedTransfer{
			Type: model.AdvancedTransferTypeBlindMatch,
			BlindMatchMetadata: &model.BlindMatchMetadata{
				ReferenceId:    "REF-001",
	...
	response, err := svc.CreateAdvancedTransfer(context.Background(), request)
	if err != nil {
		log.Fatalf("unable to create advanced transfer: %v", err)
	}
