...
func main() {
	...
	svc := advancedtransfers.NewAdvancedTransfersService(restClient)

	if len(os.Args) < 2 {
		log.Fatalf("advanced transfer id is required as a command-line argument")
	}
	advancedTransferId := os.Args[1]

	request := &advancedtransfers.CancelAdvancedTransferRequest{
		PortfolioId:        credentials.PortfolioId,
		AdvancedTransferId: advancedTransferId,
	}

	response, err := svc.CancelAdvancedTransfer(context.Background(), request)
	if err != nil {
		log.Fatalf("unable to cancel advanced transfer: %v", err)
	}
