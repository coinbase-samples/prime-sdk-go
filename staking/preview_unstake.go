...
) (*PreviewUnstakeResponse, error) {

	path := fmt.Sprintf("/portfolios/%s/wallets/%s/staking/unstake/preview", request.PortfolioId, request.WalletId)

	response := &PreviewUnstakeResponse{Request: request}
