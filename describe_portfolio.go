...
type DescribePortfolioResponse struct {
	...
	Request   *DescribePortfolioRequest `json:"request"`
}

func (c Client) DescribePortfolio(
	ctx context.Context,
	request *DescribePortfolioRequest,
) (*DescribePortfolioResponse, error) {

	path := fmt.Sprintf("/portfolios/%s", request.PortfolioId)

	response := &DescribePortfolioResponse{Request: request}
